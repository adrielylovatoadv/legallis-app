...
const ABAS_MODELO = [
  ...
  { nome: "Finalizados",       campos: "Tipo *, Cliente *, Réu, Nº Processo, Objeto, Data Finalização, Motivo — se Execução: Valor, Honorários, Repasse, Mês, Data Pagamento, Status" },
  { nome: "Acordos",           campos: "Mês, Data Pagamento, Cliente *, Réu, Nº Processo, Objeto, Valor Acordo, Honorários, Repasse, Status" },
];
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="font-serif text-2xl font-semibold" style={{ color: "var(--text)" }}>Importar Dados</h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--text3)" }}>
            Suba um arquivo Excel com os dados do escritório para importar em massa
          </p>
        </div>
        <button
      ...
      <div className="rounded-xl p-4 space-y-3" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
        <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--text3)" }}>
          A planilha modelo tem 5 abas — preencha apenas as que precisar:
        </p>
      ...
      <div
        ...
        className="rounded-xl p-10 cursor-pointer transition-all flex flex-col items-center gap-3"
        style={{
          border: `2px dashed ${file || over ? "var(--gold)" : "var(--border)"}`,
      ...
      >
        ...
        {file ? (
          ...
          <div className="text-center">
            ...
            <p className="text-xs mt-1" style={{ color: "var(--text3)" }}>
              Formato .xlsx · use a planilha modelo acima
            </p>
          </div>
        )}
      </div>
        ...
        <div className="rounded-xl p-5 space-y-4" style={{ background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)" }}>
          ...
          {result.erros && result.erros.length > 0 && (
            <div className="rounded-lg p-3 text-xs space-y-1"
              ...
              <p className="font-semibold">⚠️ Linhas ignoradas ({result.erros.length}):</p>

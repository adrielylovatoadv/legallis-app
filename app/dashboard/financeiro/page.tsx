...
function ReceitasSociosView() {
  ...
  useEffect(() => {
    Promise.all([getConfig(), getAcordos(), getExecucoes(), getHonIniciais()])
      .then(([c, a, e, h]) => {
        setConfig(c);
        setAcordos(a);
        setExecucoes(e);
    ...
    return (
      <div className="space-y-6 max-w-2xl">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Card>
            <p className="text-xs mb-1" style={{ color: "var(--text3)" }}>Total recebido</p>
            <p className="text-xl font-bold tabular-nums" style={{ color: "#4ade80" }}>{fmtBRL(totalRecebido)}</p>
          </Card>
          <Card>
            <p className="text-xs mb-1" style={{ color: "var(--text3)" }}>Total pendente</p>
            <p className="text-xl font-bold tabular-nums" style={{ color: "#f87171" }}>{fmtBRL(totalPendente)}</p>
          </Card>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <p className="text-xs mb-1" style={{ color: "var(--text3)" }}>Total recebido</p>
          <p className="text-lg font-bold tabular-nums" style={{ color: "#4ade80" }}>{fmtBRL(totalRecebido)}</p>
        </Card>
        <Card>
          <p className="text-xs mb-1" style={{ color: "var(--text3)" }}>Total pendente</p>
        ...
        <Card>
          ...
          <p className="text-lg font-bold tabular-nums" style={{ color: "var(--gold)" }}>{fmtBRL(totalRecebido)}</p>
        </Card>
        <Card>
          <p className="text-xs mb-1" style={{ color: "var(--text3)" }}>Sócios cadastrados</p>
          <p className="text-lg font-bold tabular-nums" style={{ color: "var(--text)" }}>{socios.length}</p>
        </Card>
      </div>
      ...
      <div className="space-y-3">
        ...
        {socios.map(s => {
          const valorDevido = (totalRecebido * s.percentual) / 100;
          const valorPendente = (totalPendente * s.percentual) / 100;
          return (
            <Card key={s.id}>
              <div className="flex flex-col md:flex-row md:items-center gap-4">
                ...
                <div className="flex-1 grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                  <div>
                    <p className="text-xs mb-0.5" style={{ color: "var(--text3)" }}>Valor recebido</p>
                    <p className="font-semibold tabular-nums" style={{ color: "#4ade80" }}>{fmtBRL(valorDevido)}</p>
                  </div>
                  <div>
                    <p className="text-xs mb-0.5" style={{ color: "var(--text3)" }}>Valor pendente</p>
                    <p className="font-semibold tabular-nums" style={{ color: "#f87171" }}>{fmtBRL(valorPendente)}</p>
                  </div>
                  <div>
                    <p className="text-xs mb-0.5" style={{ color: "var(--text3)" }}>Total a receber</p>
                    <p className="font-semibold tabular-nums" style={{ color: "var(--gold)" }}>{fmtBRL(valorDevido + valorPendente)}</p>
                  </div>
                </div>
                <div className="flex-shrink-0">
                  <div className="h-2 w-32 rounded-full overflow-hidden" style={{ background: "var(--surface2)" }}>
                    <div className="h-full rounded-full" style={{ width: `${s.percentual}%`, background: "var(--gold)" }} />
                  </div>
                  <p className="text-xs mt-1 text-center" style={{ color: "var(--text3)" }}>{s.percentual}%</p>
                </div>
              </div>
            </Card>
          );
      ...
      {/* Detalhe por tipo */}
      <Card>
        <h3 className="font-semibold text-sm mb-3" style={{ color: "var(--text)" }}>Detalhamento por tipo de honorário</h3>
        <div className="grid grid-cols-3 gap-4 text-center">
          {[
            ...
            <div key={item.label}>
              ...
              <p className="font-bold tabular-nums text-sm" style={{ color: "var(--gold)" }}>{fmtBRL(item.val)}</p>
            </div>
          ))}
        </div>
      </Card>
